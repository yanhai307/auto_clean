...
static char *conf_dirname = nullptr;
...
ConfYamlSetConfDirname(const char *filename)
{
    ...
    if (ep == nullptr) {
        conf_dirname = strdup(".");
        if (conf_dirname == nullptr) {
            printf("ERROR: Failed to allocate memory while loading configuration.");
            exit(EXIT_FAILURE);
        }
    }
    else {
...
ConfYamlHandleInclude(ConfNode *parent, const char *filename)
{
    ...
    if (file == nullptr) {
        printf("Failed to open configuration include file %s: %s",
    ...
    if (ConfYamlParse(&parser, parent, 0) != 0) {
        printf("Failed to include configuration file %s", filename);
        return -1;
    }
...
//    printf("parent: %p, name: %s\n", parent, parent->name);
    while (!done) {
        if (!yaml_parser_parse(parser, &event)) {
            printf("Failed to parse configuration file at line %" PRIuMAX ": %s\n",
                (uintmax_t)parser->problem_mark.line, parser->problem);
            return -1;
        }

        if (event.type == YAML_DOCUMENT_START_EVENT) {
//            printf("event.type=YAML_DOCUMENT_START_EVENT; state=%d\n", state);
...
//            printf("event.type=YAML_SCALAR_EVENT; state=%d; value=%s; "
//                "tag=%s; inseq=%d\n", state, value, tag, inseq);
            if (inseq) {
                ...
                if (seq_node != nullptr) {
                    ...
                    if (unlikely(seq_node == nullptr)) {
                        return -1;
                    }
                    seq_node->name = strdup(sequence_node_name);
                    if (unlikely(seq_node->name == nullptr)) {
                        free(seq_node);
                        return -1;
                    }
                    seq_node->val = strdup(value);
                    if (unlikely(seq_node->val == nullptr)) {
                        free(seq_node->name);
                        return -1;
                    }
                }
...
//                                existing->name);
                        ...
                        if (node->name && strchr(node->name, '_')) {
                            if (!(parent->name &&
                                ...
                                if (mangle_errors < MANGLE_ERRORS_MAX) {
                                    printf("WARN: %s is deprecated. Please use %s on line %" PRIuMAX".\n",
                                            value, node->name, (uintmax_t)parser->mark.line+1);
                                    mangle_errors++;
                                    if (mangle_errors >= MANGLE_ERRORS_MAX)
                                        printf("WARN: not showing more parameter name warnings.\n");
                                }
                            }
                        }
                    ...
                    if ((tag != nullptr) && (strcmp(tag, "!include") == 0)) {
//                        printf("INFO: Including configuration file %s at "
//                            "parent node %s.\n", value, node->name);
...
//            printf("event.type=YAML_SEQUENCE_START_EVENT; state=%d", state);
...
//            printf("event.type=YAML_SEQUENCE_END_EVENT; state=%d", state);
            return 0;
        }
        else if (event.type == YAML_MAPPING_START_EVENT) {
//            printf("event.type=YAML_MAPPING_START_EVENT; state=%d", state);
            if (inseq) {
                char sequence_node_name[DEFAULT_NAME_LEN];
                snprintf(sequence_node_name, DEFAULT_NAME_LEN, "%d", seq_idx++);
...
//            printf("event.type=YAML_MAPPING_END_EVENT; state=%d", state);
            done = 1;
        }
        else if (event.type == YAML_STREAM_END_EVENT) {
//            printf("event.type=YAML_STREAM_END_EVENT; state=%d", state);
            done = 1;
        }
...
ConfYamlLoadFile(const char *filename)
{
    ...
    if (stat(filename, &stat_buf) == 0) {
        if (stat_buf.st_mode & S_IFDIR) {
            printf("yaml argument is not a file but a directory: %s. "
...
ConfYamlLoadFileWithPrefix(const char *filename, const char *prefix)
{
    ...
    infile = fopen(filename, "r");
    if (infile == nullptr) {
        printf("failed to open file: %s: %s", filename,
            strerror(errno));
        yaml_parser_delete(&parser);
        return -1;
    }
...
ConfYamlFileIncludeTest(void)
{
    ...
    if ((config_file = fopen(config_filename, "w")) == nullptr) {
        goto cleanup;
    }
    ...
    fclose(config_file);
    if ((config_file = fopen(include_filename, "w")) == nullptr) {
        goto cleanup;
    }
...
ConfYamlOverrideTest(void)
{
    ...
    if (ConfGetNode("parent.child0") != nullptr)
        return 0;
